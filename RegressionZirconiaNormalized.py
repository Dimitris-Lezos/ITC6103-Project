...
data = data[data['carat'] <= 0.54]
data = data[0.39 <= data['depth']]
data = data[data['depth'] <= 0.62]
data = data[0.135 <= data['table']]
data = data[data['table'] <= 0.41]
print('Non Ouliers Data: ', data.shape)
...
# #### 5.5 Boxplots
# plt.figure(4)
# plot = sns.boxplot(data=data.drop(columns=['price', 'price_carat']))
# plt.figure(41)
# scaler = MinMaxScaler()
# box_data = data.drop(columns=['price', 'price_carat'])
# box_data = pd.DataFrame(scaler.fit_transform(box_data), index = box_data.index, columns = box_data.columns)
# plot = sns.boxplot(data=box_data)
# plt.show()
#exit(0)
...
x_train_drop = x_train.copy()
x_test_drop = x_test.copy()
x_train_drop.drop(columns=['depth', 'cut'], inplace=True)
x_test_drop.drop(columns=['depth', 'cut'], inplace=True)
...
mse = mean_squared_error(y_test['price'], y_pred)
print('------------------------')
print("['carat', 'cut', 'color', 'clarity', 'table'] Ridge Poly 8 MSE=", round(mse, 2))
print("['carat', 'cut', 'color', 'clarity', 'table'] Ridge Poly 8  R2=", r2_score(y_test['price'], y_pred))
